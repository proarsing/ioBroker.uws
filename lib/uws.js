...
class IOWebSocketSrv extends EventEmitter {
   ...
   */
  initIOServer() {
    this._adapter.log.info('Initializing uWebSocket server...');
   ...
   */
  close() {
    if (this._LOG_ALL) this._adapter.log.info('Gracefully closing uWebSockets server');
    ...
    if ( this.server && this.SOCKETS && this.SOCKETS.sockets ) {
      ...
      try {
        if (this.listenSocket) {
          ...
          // this._adapter.log.info('Okay, shutting down Server now!');
...
class IOSocketC {
  ...
  stateSubscribe(id, socketConnectionId) {
    ...
    this.readSingleState(id, /*callback*/(point)=> {
      if (point && !point.err) {
        if (_that._subscribeStates[socketConnectionId].indexOf(id) === -1) {
            if ( Array.isArray(_that._subscribeStates[socketConnectionId]) ) { 
              _that._subscribeStates[socketConnectionId].push(id);
              _that.onForeignStateSubscribe(_that, id);
            }
            //this.onForeignStateSubscribe(_that, id);
        }
        // _that._printSubscribedStates();
  ...
  readSingleState(id, readSingleStateCallback = null) {
    this._adapter.getForeignState(id, /*{
        ...
        if ( error || !res || res === null || (res && (res.ack === undefined || res.ts === undefined || res.from === undefined || res.val === undefined)) ) {
          ...
          Result = {
            ...
            state: {
              // id,
              // value: (res.val !== null) ? res.val : '(null)',
              // ack: (res.ack) ? res.ack : 'n/a',
              // timestamp: (res.ts) ? new Date(res.ts).toISOString() : '####-##-##',
              // q: (res.q) ? res.q : 'null',
              // from: (res.from) ? res.from : 'n/a',
              // user: (res.user) ? res.user : 'n/a',
              // lastchange: (res.lc) ? new Date(res.lc).toISOString() : '####-##-##'
              id,
              value: res.val,
              ack: res.ack,
