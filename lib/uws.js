...
class IOWebSocket extends EventEmitter {
  ...
  constructor(adapter = null, port, log, authSettings = {}) {
    ...
    if ( !(this instanceof IOWebSocket) ) {
      return new IOWebSocket(adapter, port, log, authSettings);
    }
  ...
  initIOServer() {
    ...
    this._adapter.log.info('[initIOServer] uWS = ' + uWS);
    ...
    this.server = uWS.App({
        ...
        this._adapter.log.info('[listen] listenSocket = ' + token);
        this._adapter.log.info(`[listen] port = ${this._port}`);
        // token ?
        // this._adapter.log.info('WebSockets Server listening to port ' + this._port) :
        // this._adapter.log.error("WebSockets Server failed to listen to port " + this._port);
        if ( token ) {
          this._adapter.log.info('WebSockets Server listening to port ' + this._port)
        } else {
          this._adapter.log.error('WebSockets Server failed to listen to port ' + this._port);
      ...
      this.server = uWS./*SSL*/App({
        ...
        message: (ws, message, isBinary) => { this._adapter.log.info(message) },
        close: (ws, code, message) => {this._adapter.log.info('socket closed.')}
        })
        .get('/*', (res, req) => {
          res.end('Hello World!');
      }).listen(this._port, (token) => {
        if (token) {
          ...
          this._adapter.log.info('Emitting connected...');
