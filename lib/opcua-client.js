...
// helper function to return Random Integer Between X and Y (Inclusive Y)
...
async function init(endpointUrl, _adapter, _log) {
    LOG_ALL = _log;
    adapter = _adapter;

    const rndNum = _rnd(10,999) + "";
    const clientName = "ioBroker-" + rndNum + "-client";
            ...
            monitoredItem.on("changed", (dataValue) => {
                let Node = {
                  data: {
                    ...
                    type: _typeOfNode(dataValue.value.dataType),
...
async function readOpcNode(nodeid, cb) {
    ...
    let Result = {
        ...
        type: _typeOfNode(opcValue.value.dataType),
